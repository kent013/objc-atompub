    ...
    switch (mode) {
        ...
        case PostingEntry:
            ...
            [ self handleResponseForPostingResourceWithDispatcher:
             @selector(client:didCreateEntry:withLocation:) ];
            break;
        case PostingMedia:
            NSLog(@"Posted media");
            [ self handleResponseForPostingResourceWithDispatcher:
             @selector(client:didCreateMediaLinkEntry:withLocation:) ];
            break;
        case PuttingEntry:
            NSLog(@"Putted entry");
...
- (void)handleResponseForPostingResourceWithDispatcher:(SEL)dispatcher {
    ...
    if (status >= 200 && status < 300) {
        ...
        if (doc == nil) {
            ...
            if ( delegate != nil
                && [ delegate respondsToSelector:dispatcher ] )
                objc_msgSend(delegate, dispatcher, [ [ self retain ] autorelease ], [ AtomEntry entryWithXMLElement:[ doc rootElement ] ], [ NSURL URLWithString:location ] );
        }
    } else {
        [ self dispatchErrorWithStatus:status
                           description:message ];
...
- (void)handleResponseForGettingEntry {
    ...
    if (status == 200) {
        ...
        if (doc == nil) {
            ...
            if (delegate != nil
                && [ delegate respondsToSelector:@selector(client:didReceiveEntry:) ] ) {
                [ delegate performSelector:@selector(client:didReceiveEntry:)
                                withObject:[ [ self retain ] autorelease ]
...
- (void)handleResponseForGettingMedia {
    ...
    if (status == 200) {
        ...
        if (delegate != nil
            && [ delegate respondsToSelector:@selector(client:didReceiveMedia:) ] ) {
            [ delegate performSelector:@selector(client:didReceiveMedia:)
                            withObject:[ [ self retain ] autorelease ]
...
- (void)handleResponseWithType:(NSString *)aType
    ...
    if (status == 200) {
        ...
        if (doc == nil) {
            ...
            if (delegate != nil
                && [ delegate respondsToSelector:dispatcher ] ) {
                objc_msgSend(delegate, dispatcher, [ [ self retain ] autorelease ], [ [ [ class alloc ] initWithXMLElement:[ doc rootElement ] ] autorelease ]);
            }
        }
    } else {
        [ self dispatchErrorWithStatus:status
